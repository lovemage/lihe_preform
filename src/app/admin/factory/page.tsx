...
import { getFactoryContent, listMedia } from "@/lib/admin/content-repository";

export const dynamic = "force-dynamic";

export default async function AdminFactoryPage() {
  await requireAdminSession();
  const [en, ru, es, media] = await Promise.all([
  ...
  return (
    <AdminShell title="Factory 頁面" description="管理工廠頁面的內文、段落與圖片資產。">
      <FactoryEditor initialContent={{ en: en.content, ru: ru.content, es: es.content }} media={media} />
    </AdminShell>
  );
}
